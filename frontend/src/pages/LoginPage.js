...
const LoginPage = ({ setIsLoggedIn }) => {
  ...
  const handleLogin = async () => {
    ...
    try {
      setLoading(true);
      const response = await axios.post('/auth/login', { email, password, role: 'client' });
  ...
  return (
    ...
    >
          ...
          endAdornment: (
            <InputAdornment position="end">
              <IconButton
                ...
                edge="end"
              >
                {showPassword ? <VisibilityOff /> : <Visibility />} {/* Biểu tượng thay đổi */}
              </IconButton>
            </InputAdornment>
          ),
        }}
      ...
      >
        {loading ? <CircularProgress size={21} color="inherit" /> : 'Đăng Nhập'}
      </Button>
      ...
      >
        <img src={facebookIcon} alt="Facebook" style={{ height: 24, marginRight: 8 }} />
