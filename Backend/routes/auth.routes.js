...
router.get(
  ...
  (req, res) => {
    const token = jwt.sign({ id: req.user.id, role: 'client' }, process.env.JWT_SECRET, { expiresIn: '1d' });
...
router.post('/google', async (req, res) => {
  ...
  try {
    ...
    let user = await Client.findOne({ google_id: sub });
    if (!user) {
    ...
    const jwtToken = jwt.sign({ id: user._id, role: 'client' }, process.env.JWT_SECRET, { expiresIn: '1d' });
...
router.get(
  ...
  async (req, res) => {
    try {
      const token = jwt.sign({ id: req.user.id, role: 'client' }, process.env.JWT_SECRET, { expiresIn: '1d' });
      res.redirect(`${process.env.FRONTEND_URL}/login-success?token=${token}`); // Chuyển token đến frontend
    } catch (error) {
